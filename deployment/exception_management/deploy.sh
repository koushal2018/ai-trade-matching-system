#!/bin/bash
# Deployment script for Exception Management Agent to AgentCore Runtime
# Requirements: 2.1, 2.2, 3.5

set -e

echo "====================================================="
echo "Deploying Exception Management Agent to AgentCore"
echo "====================================================="

# Configuration
AGENT_NAME="exception_management_agent"
RUNTIME="PYTHON_3_11"
REGION="${AWS_REGION:-us-east-1}"
S3_BUCKET="${S3_BUCKET_NAME:-trade-matching-system-agentcore-production}"
ENTRYPOINT="exception_management_agent.py"
...
if [ -n "${AGENTCORE_MEMORY_ARN}" ]; then
    ...
    echo "Memory Strategy: event,semantic (both for exception patterns and RL policies)"
else
    echo ""
...
agentcore launch \
  ...
  --env "DYNAMODB_EXCEPTIONS_TABLE=ExceptionsTable" \
...
echo ""
echo "====================================================="
echo "Exception Management Agent deployed successfully!"
echo "====================================================="
echo ""
